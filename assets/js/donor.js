...
export class DonorRuntime{
  ...
  async load(onProgress=null){
    ...
    progress(0.02,'Loading donor manifest…');
    this.manifest=await fetch(`${this.base}/manifest.json`,{cache:'force-cache'}).then(r=>{if(!r.ok)throw Error('Failed to load donor manifest');return r.json()});

    const entries=Object.entries(this.manifest.actions);
    for(let i=0;i<entries.length;i++){
      const [k,m]=entries[i];
      progress(i/entries.length,`Loading donor support A${k}…`);
      this.actions[k]={
        m,
        emb:await fetchF32(`${this.base}/${m.embeddings}`),
        out:await fetchF32(`${this.base}/${m.outcomes}`),
        grp:await fetchI32(`${this.base}/${m.groups}`)
      };
      progress((i+1)/entries.length,`Donor support A${k} ready`);
    }

import {parseTimelineCSV,prepareInput} from './preprocess.js?v=4.1';
...
function updateRunState(){
  ...
  const ready=fieldsOk && fileValid && timeline.length>0 && contractsReady;
  btn.disabled=!ready;
  ...
  if(!contractsReady){
    hint.textContent='Calculator runtime is still being checked.';
...
async function loadContracts(){
  ...
  const s=await fetch('data/policy_summary.json',{cache:'no-store'}).then(r=>r.json()).catch(()=>null);
...
async function run(e){
  ...
  if(!contractsReady){
    setText('formMessage','Calculator runtime is not ready yet. Please wait a moment and try again.');
    updateRunState();
    return;
  }
  ...
  setText('readyHint','Loading the frozen V17 runtime and calculating the recommendation…');
...
(async()=>{
  ...
  try{
    ...
    setText('formMessage','The CSV uploader is available, but the calculator runtime could not be initialized. Confirm that the runtime/ assets are present.');
  }finally{
    updateRunState();
  }
})();

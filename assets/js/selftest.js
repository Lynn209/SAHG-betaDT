...
export async function runBrowserSelfTest({model,donor,norm,structuralProfiles,gate,lock,policy,oodRef,onProgress=null}){
  ...
  const fixture=await fetch('runtime/browser_selftest.json?v=7.1',{cache:'no-store'}).then(r=>{if(!r.ok)throw Error('Missing runtime/browser_selftest.json');return r.json()});
  ...
  const preprocessTol=Math.max(Number(preprocessTol),1e-5);
